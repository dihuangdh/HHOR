...
class SDFNetwork(nn.Module):
    def __init__(self,
        ...
        # semantic branch
        lin = nn.Linear(dims[-1]-1, n_classes)
        setattr(self, 'lin_sem', lin)
        self.activation_sem = nn.Sigmoid()
    ...
    def forward(self, inputs):  # (batch_size*n_samples, 3)
        ...
        # semantic branch
        conf = self.lin_sem(x[:, 1:])
        conf = self.activation_sem(conf)

        return torch.cat([x[:, :1] / self.scale, conf, x[:, 1:]], dim=-1)  # (batch_size*n_samples, d_out)

    def sdf(self, x):
        return self.forward(x)[:, :1]
    
    def conf(self, x):
        return self.forward(x)[:, 1:self.n_classes+1]
...
class RenderingNetwork(nn.Module):
    ...
    def forward(self, points, normals, view_dirs, feature_vectors, time):
        ...
        lcode = lcode[:, None, :].repeat(1, int(rendering_input.shape[0]/time.shape[0]), 1)
        lcode = lcode.reshape(-1, 10)
        rendering_input = torch.cat([rendering_input, lcode], -1)
